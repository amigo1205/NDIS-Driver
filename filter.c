...
BOOLEAN CheckSentIPv4Header(PNET_BUFFER NetBuffer);
BOOLEAN CheckTCPUDPPacket(PNDF_IPV4_HEADER IPv4Header, PNET_BUFFER NetBuffer);
...
--*/
{
    ...
    do
    {
        ...
        Status = FilterRegisterDevice(&pFilterDeviceExtension);
...
BOOLEAN CheckSentPacket(PNET_BUFFER NetBuffer)
{
	...
	{
		case ETHERTYPE_IP4:
			IsValid = CheckSentIPv4Header(NetBuffer);
			break;
		case ETHERTYPE_ARP:
			NdisAdvanceNetBufferDataStart(NetBuffer, sizeof(NDF_ETH_HEADER), FALSE, NULL);
			DataOffset = sizeof(NDF_ETH_HEADER);
...
BOOLEAN CheckSentIPv4Header(PNET_BUFFER NetBuffer)
{
	...
	while (CurrentRule != NULL)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			tmp = (pIPv4Header->DstIp[i] == CurrentRule->IP[i]);
			InRange = InRange && tmp;
		}
		tmp = (pIPv4Header->Protocol == CurrentRule->Protocol);
		InRange = InRange && tmp;

		if (pIPv4Header->Protocol == 6) //tcp
		{
			NdisAdvanceNetBufferDataStart(NetBuffer, sizeof(NDF_IPV4_HEADER), FALSE, NULL);
			PNDF_TCP_HEADER pTcpHeader = (PNDF_TCP_HEADER)NdisGetDataBuffer(NetBuffer, sizeof(PNDF_TCP_HEADER), NULL, 1, 0);
			tmp = (pTcpHeader->DstPort == CurrentRule->Port);
			InRange = InRange && tmp;
			NdisRetreatNetBufferDataStart(NetBuffer, sizeof(NDF_IPV4_HEADER), 0, NULL);
		}
		else if (pIPv4Header->Protocol == 17) //udp
		{
			NdisAdvanceNetBufferDataStart(NetBuffer, sizeof(NDF_IPV4_HEADER), FALSE, NULL);
			PNDF_UDP_HEADER pUdpHeader = (PNDF_UDP_HEADER)NdisGetDataBuffer(NetBuffer, sizeof(PNDF_UDP_HEADER), NULL, 1, 0);
			tmp = (pUdpHeader->destinationPort == CurrentRule->Port);
			InRange = InRange && tmp;
			NdisRetreatNetBufferDataStart(NetBuffer, sizeof(NDF_IPV4_HEADER), 0, NULL);
		}
		IsValid = InRange;
...
BOOLEAN CheckSentArpHeader(PNDF_ARP_HEADER ArpHeader)
{
	...
	while (CurrentRule != NULL)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			BOOLEAN tmp1 = (ArpHeader->DstIp[i] == CurrentRule->IP[i]);
			InRange = InRange && tmp1;
		}
		IsValid = InRange;
		if (IsValid)
			break;
...
BOOLEAN CheckReceiveIPv4Header(PNDF_IPV4_HEADER IPv4Header)
{
	...
	while (CurrentRule != NULL)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			BOOLEAN tmp1 = (IPv4Header->SrcIp[i] == CurrentRule->IP[i]);
			InRange = InRange && tmp1;
		}
		BOOLEAN tmp2 = (IPv4Header->Protocol == CurrentRule->Protocol);
		InRange = InRange && tmp2;
		IsValid = InRange;
		if (IsValid)
			break;
		CurrentRule = CurrentRule->Next;
...
BOOLEAN CheckReceiveArpHeader(PNDF_ARP_HEADER ArpHeader)
{
	...
	while (CurrentRule != NULL)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			BOOLEAN tmp1 = (ArpHeader->SrcIp[i] == CurrentRule->IP[i]);
			InRange = InRange && tmp1;
		}
		IsValid = InRange;
		if (IsValid)
